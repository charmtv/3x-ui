...
EOF
            umask "${prev_umask}"
            echo -e "${red}无法写入 PostgreSQL 凭据到 ${PG_CRED_FILE}${plain}" >&2
            return 1
        fi
        umask "${prev_umask}"
    fi
...
ensure_pg_client() {
    ...
    echo -e "${yellow}正在安装面板备份所需的 PostgreSQL 客户端工具（pg_dump/pg_restore）...${plain}" >&2
    case "${release}" in
        ubuntu | debian | armbian)
...
setup_ssl_certificate() {
    ...
    echo -e "${yellow}提示：80 端口必须开放，并且可从公网访问${plain}"
    ...
    if [ $? -ne 0 ]; then
        ...
        echo -e "${yellow}请确认 80 端口已开放，稍后可运行 x-ui 重试${plain}"
...
setup_ip_certificate() {
    ...
    echo -e "${green}正在配置 Let's Encrypt IP 证书（短有效期配置）...${plain}"
    echo -e "${yellow}提示：IP 证书有效期约 6 天，并会自动续期。${plain}"
    echo -e "${yellow}默认监听 80 端口。如选择其他端口，请确保外部 80 端口已转发到该端口。${plain}"
    ...
    if [[ "${WebPort}" -ne 80 ]]; then
        echo -e "${yellow}提醒：Let's Encrypt 仍会连接外部 80 端口；请将外部 80 端口转发到 ${WebPort}。${plain}"
    fi
    ...
    echo -e "${green}正在为 IP ${ipv4} 签发证书...${plain}"
    ~/.acme.sh/acme.sh --set-default-ca --server letsencrypt --force > /dev/null 2>&1
    ...
    echo -e "${yellow}acme.sh 会在证书过期前自动续期并重载 x-ui。${plain}"
    return 0
}
...
ssl_cert_issue() {
    ...
    if ~/.acme.sh/acme.sh --list 2> /dev/null | awk '{print $1}' | grep -Fxq "${domain}"; then
        ...
        if [[ -n "${acmeCertDir}" ]]; then
            ...
            echo -e "${yellow}发现 ${domain} 已有证书，将复用该证书。${plain}"
    ...
    if [[ -z ${WebPort} ]]; then
        ...
        echo -e "${yellow}输入的端口 ${WebPort} 无效，将使用默认 80 端口。${plain}"
        WebPort=80
    fi
    ...
    echo -e "${green}每次证书签发和续期都会执行此命令。${plain}"
    if [[ "$NONINTERACTIVE" == "1" ]]; then
        setReloadcmd="n"
    else
...
prompt_and_setup_ssl() {
    ...
    echo -e "${green}4.${plain} 跳过 SSL（高级选项，仅适用于反向代理或 SSH 隧道）"
    echo -e "${blue}提示：${plain}选项 1 和 2 需要开放 80 端口；选项 3 需要手动提供证书路径。"
    echo -e "${blue}提示：${plain}选项 4 将通过明文 HTTP 提供面板，仅可在 nginx/Caddy 或 SSH 隧道后使用。"
    if [[ "$NONINTERACTIVE" == "1" ]]; then
        case "${XUI_SSL_MODE:-none}" in
            ...
            none | "") ssl_choice="4" ;;
            *)
                echo -e "${yellow}未知的 XUI_SSL_MODE='${XUI_SSL_MODE}'，将默认使用无 SSL 的 HTTP 模式。${plain}"
                ssl_choice="4"
                ;;
        esac
    else
    ...
    case "$ssl_choice" in
        1)
            ...
            if ssl_cert_issue; then
                ...
                echo -e "${red}域名证书模式配置 SSL 失败。${plain}"
                SSL_HOST="${server_ip}"
            fi
            ;;
        2)
            ...
            if [[ "$NONINTERACTIVE" != "1" ]]; then
                local ip_confirm=""
                read -rp "${server_ip} 是否为此服务器正确的公网 IPv4 地址？[默认 y]：" ip_confirm
            ...
            local ipv6_addr=""
            prompt_or_default ipv6_addr "是否需要加入 IPv6 地址？（留空跳过）：" "" XUI_SSL_IPV6
        ...
        3)
            ...
            while true; do
                read -rp "请输入证书路径（关键字：.crt / fullchain）：" custom_cert
            ...
            while true; do
                read -rp "请输入私钥路径（关键字：.key / privatekey）：" custom_key
                # Strip quotes if present
            ...
            echo -e "${yellow}提示：你需要自行在外部续期这些文件。${plain}"

            systemctl restart x-ui > /dev/null 2>&1 || rc-service x-ui restart > /dev/null 2>&1
            ;;
        4)
            echo ""
            echo -e "${red}⚠ 面板将不启用 SSL/TLS。${plain}"
            echo -e "${yellow}登录凭据和 Cookie 将通过明文 HTTP 传输。${plain}"
            echo -e "${yellow}仅在以下情况才安全：${plain}"
            echo -e "${yellow}  • 由反向代理（nginx、Caddy、Traefik）负责 TLS 终止，或${plain}"
            ...
            if [[ "$NONINTERACTIVE" == "1" ]]; then
                ...
                read -rp "是否仅将面板绑定到 127.0.0.1？（推荐，将强制通过 SSH 隧道或反向代理访问）[y/N]：" bind_local
            fi
            if [[ "$bind_local" == "y" || "$bind_local" == "Y" ]]; then
                ...
                echo ""
                echo -e "${green}SSH 端口转发：在本机通过以下方式打开面板：${plain}"
                echo -e "  标准 SSH 命令："
                ...
                echo ""
                echo -e "${yellow}也可以将反向代理（nginx/Caddy）指向 127.0.0.1:${panel_port} 并由它处理 TLS。${plain}"
            else
                echo -e "${yellow}面板将在所有网卡上监听明文 HTTP，请确保前置服务负责 TLS 终止。${plain}"
            fi
...
config_after_install() {
    ...
    if [[ ${#existing_webBasePath} -lt 4 ]]; then
        if [[ "$existing_hasDefaultCredential" == "true" ]]; then
            ...
            echo -e "  2) PostgreSQL （建议大量客户端或多节点时使用）"
            if [[ "$NONINTERACTIVE" == "1" ]]; then
            ...
            if [[ "$db_choice" == "2" ]]; then
                ...
                while [[ -z "$xui_dsn" ]]; do
                    if [[ "$NONINTERACTIVE" == "1" ]]; then
                        ...
                        echo -e "${yellow}请设置 XUI_DB_DSN 使用现有服务器，或设置 XUI_DB_TYPE=sqlite。${plain}"
                        exit 1
                    fi
                    echo ""
...
EOF
                    ...
                    ensure_pg_client || echo -e "${yellow}⚠ 无法安装 pg_dump/pg_restore。安装 postgresql-client 包之前，面板内数据库备份/恢复不可用。${plain}"
                fi
            fi
            ...
            if [[ "$NONINTERACTIVE" == "1" ]]; then
                ...
                read -rp "是否自定义面板端口？（选择否将使用随机端口）[y/n]：" config_confirm
            ...
            echo -e "${green}═══════════════════════════════════════════${plain}"
            echo -e "${yellow}强烈建议启用 SSL。只有在反向代理${plain}"
            echo -e "${yellow}或 SSH 隧道已经处理 TLS 时才跳过。${plain}"
            ...
            if [[ "$db_choice" == "2" ]]; then
                echo ""
                echo -e "${green}面板内置 PostgreSQL 备份与恢复：${plain}"
            ...
            if [[ "$db_choice" == "2" && "$pg_local_installed" == "1" ]]; then
                ...
                echo -e "${green}═══════════════════════════════════════════${plain}"
                echo -e "${yellow}⚠ 面板会从 ${xui_env_file} 读取这些凭据。${plain}"
            ...
            echo -e "${yellow}WebBasePath 缺失或过短，正在生成新的访问路径...${plain}"
...
setup_fail2ban() {
    if [[ -n "${XUI_ENABLE_FAIL2BAN+x}" && "${XUI_ENABLE_FAIL2BAN}" != "true" ]]; then
        echo -e "${yellow}XUI_ENABLE_FAIL2BAN=${XUI_ENABLE_FAIL2BAN}，正在跳过 Fail2ban 自动配置。${plain}"
        return 0
    fi

    if [[ ! -x /usr/bin/x-ui ]]; then
        echo -e "${yellow}未找到 x-ui 命令，正在跳过 Fail2ban 自动配置。${plain}"
        return 0
    fi
    ...
    if /usr/bin/x-ui setup-fail2ban; then
        ...
        echo -e "${yellow}Fail2ban 配置未完成；在运行 x-ui 并打开 IP 限制菜单前，该功能将保持禁用。安装将继续。${plain}"
    fi
    return 0
}
...
install_x-ui() {
    ...
    if [ $# == 0 ]; then
        ...
        if [[ ! -n "$tag_version" ]]; then
            echo -e "${red}获取 x-ui 版本失败，可能受到 GitHub API 限制，请稍后重试。${plain}"
            exit 1
        fi
        ...
        if [[ "$tag_version" == "dev" || "$tag_version" == "dev-latest" ]]; then
            ...
            echo -e "${yellow}正在安装滚动开发版（标签：dev-latest）。这是随提交更新的预发布版本，不是稳定版。${plain}"
        else
    ...
    curl -fLRo "${xui_script_temp}" https://raw.githubusercontent.com/charmtv/3x-ui/main/x-ui.sh
    if [[ $? -ne 0 ]]; then
        rm -f "${xui_script_temp}"
    ...
    if [[ $? -ne 0 ]]; then
        ...
        rm -f "${xui_script_temp}"
        echo -e "${red}解压 x-ui 发布包失败。旧安装已被移除，修复前面板无法启动，请重新运行安装器。${plain}"
        exit 1
    fi
    rm x-ui-linux-$(arch).tar.gz -f
    ...
    if [[ $? -ne 0 || ! -s x-ui ]]; then
        rm -f "${xui_script_temp}"
        echo -e "${red}解压后的 x-ui 发布包缺少 x-ui 二进制文件。旧安装已被移除，修复前面板无法启动，请重新运行安装器。${plain}"
        exit 1
    fi
    chmod +x x-ui
    chmod +x x-ui.sh
    ...
    if [[ $release == "alpine" ]]; then
        ...
        rm -f "${xui_rc_temp}"
        curl -fLRo "${xui_rc_temp}" https://raw.githubusercontent.com/charmtv/3x-ui/main/x-ui.rc
        if [[ $? -ne 0 ]]; then
            rm -f "${xui_rc_temp}"
        ...
        if [ "$service_installed" = false ]; then
            ...
            case "${release}" in
                ubuntu | debian | armbian)
                    service_unit_url="https://raw.githubusercontent.com/charmtv/3x-ui/main/x-ui.service.debian"
                    ;;
                arch | manjaro | parch)
                    service_unit_url="https://raw.githubusercontent.com/charmtv/3x-ui/main/x-ui.service.arch"
                    ;;
                *)
                    service_unit_url="https://raw.githubusercontent.com/charmtv/3x-ui/main/x-ui.service.rhel"
                    ;;
            esac
    ...
    echo ""
    echo -e "${blue}x-ui 常用命令：${plain}"
    echo -e "  ${blue}x-ui${plain}              打开简体中文管理菜单"
    ...
    echo -e "  ${blue}x-ui update-dev${plain}   更新开发版"
