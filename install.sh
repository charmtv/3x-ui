...
config_after_install() {
    ...
    if [[ ${#existing_webBasePath} -lt 4 ]]; then
        if [[ "$existing_hasDefaultCredential" == "true" ]]; then
            ...
            echo -e "  2) PostgreSQL （推荐用于大量客户端或多节点）"
...
EOF
            ...
            read -rp "是否自定义面板端口？（否则会随机生成端口）[y/n]：" config_confirm
            if [[ "${config_confirm}" == "y" || "${config_confirm}" == "Y" ]]; then
                read -rp "请设置面板端口：" config_port
                echo -e "${yellow}你的面板端口为：${config_port}${plain}"
            else
...
install_x-ui() {
    ...
    curl -4fLRo /usr/bin/x-ui-temp https://raw.githubusercontent.com/charmtv/3x-ui/main/x-ui.sh
    if [[ $? -ne 0 ]]; then
    ...
    if [[ $release == "alpine" ]]; then
        curl -4fLRo /etc/init.d/x-ui https://raw.githubusercontent.com/charmtv/3x-ui/main/x-ui.rc
        if [[ $? -ne 0 ]]; then
        ...
        if [ "$service_installed" = false ]; then
            ...
            case "${release}" in
                ubuntu | debian | armbian)
                    curl -4fLRo ${xui_service}/x-ui.service https://raw.githubusercontent.com/charmtv/3x-ui/main/x-ui.service.debian > /dev/null 2>&1
                    ;;
                arch | manjaro | parch)
                    curl -4fLRo ${xui_service}/x-ui.service https://raw.githubusercontent.com/charmtv/3x-ui/main/x-ui.service.arch > /dev/null 2>&1
                    ;;
                *)
                    curl -4fLRo ${xui_service}/x-ui.service https://raw.githubusercontent.com/charmtv/3x-ui/main/x-ui.service.rhel > /dev/null 2>&1
                    ;;
            esac
