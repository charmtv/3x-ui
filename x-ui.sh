...
update_menu() {
    ...
    curl -fLRo /usr/bin/x-ui https://raw.githubusercontent.com/charmtv/3x-ui/main/x-ui.sh
...
update_shell() {
    curl -fLRo /usr/bin/x-ui -z /usr/bin/x-ui https://github.com/charmtv/3x-ui/raw/main/x-ui.sh
    if [[ $? != 0 ]]; then
        echo ""
...
show_mtproto_status() {
    ...
    echo -e "  ${yellow}mtg 日志：${plain} journalctl -u x-ui --no-pager -n 200 | grep -i mtproto"
}
...
migrate_to_postgres() {
    ...
    if check_status; then
        ...
        LOGE "面板未启动。请检查日志（选项 16）。SQLite 数据保持不变。"
    fi
}
