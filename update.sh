...
update_x-ui() {
    ...
    tag_version=$(${curl_bin} -Ls "https://api.github.com/repos/charmtv/3x-ui/releases/latest" 2> /dev/null | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/')
    if [[ ! -n "$tag_version" ]]; then
        echo -e "${yellow}正在尝试使用 IPv4 获取版本...${plain}"
        tag_version=$(${curl_bin} -4 -Ls "https://api.github.com/repos/charmtv/3x-ui/releases/latest" | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/')
        if [[ ! -n "$tag_version" ]]; then
    ...
    if [[ $? -ne 0 ]]; then
        echo -e "${yellow}正在尝试使用 IPv4 获取版本...${plain}"
        ${curl_bin} -4fLRo ${xui_folder}-linux-$(arch).tar.gz https://github.com/charmtv/3x-ui/releases/download/${tag_version}/x-ui-linux-$(arch).tar.gz 2> /dev/null
        if [[ $? -ne 0 ]]; then
