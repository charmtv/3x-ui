...
fetch_latest_xui_tag() {
    ...
    tag=$(${curl_bin} -Ls "https://api.github.com/repos/${repo}/releases/latest" 2> /dev/null | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/')
    if [[ -z "$tag" ]]; then
        echo -e "${yellow}正在尝试使用 IPv4 获取 ${repo} 版本...${plain}" >&2
        tag=$(${curl_bin} -4 -Ls "https://api.github.com/repos/${repo}/releases/latest" 2> /dev/null | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/')
    fi
...
download_xui_release() {
    ...
    ${curl_bin} -fLRo "$archive" "$url" 2> /dev/null && return 0
    echo -e "${yellow}正在尝试使用 IPv4 下载 x-ui...${plain}" >&2
    ${curl_bin} -4fLRo "$archive" "$url" 2> /dev/null
}
...
fetch_latest_xui_asset_tag() {
    ...
    tag=$(${curl_bin} -fsSI "$url" 2> /dev/null | tr -d '\r' | awk 'tolower($0) ~ /^location:/ && index($0, "/releases/download/") { sub(/^.*\/releases\/download\//, "", $0); sub(/\/.*$/, "", $0); print; exit }')
    if [[ -z "$tag" ]]; then
        tag=$(${curl_bin} -4fsSI "$url" 2> /dev/null | tr -d '\r' | awk 'tolower($0) ~ /^location:/ && index($0, "/releases/download/") { sub(/^.*\/releases\/download\//, "", $0); sub(/\/.*$/, "", $0); print; exit }')
    fi
...
update_x-ui() {
    ...
    if ! download_latest_xui_release; then
        _fail "错误：获取 x-ui 版本或下载制品失败，请检查服务器网络或 release 配置。"
    fi
    ...
    if [[ -e ${xui_folder}/ ]]; then
        ...
        echo -e "${green}正在移除旧版 README 和 LICENSE 文件...${plain}"
    ...
    ${curl_bin} -fLRo /usr/bin/x-ui https://raw.githubusercontent.com/charmtv/3x-ui/main/x-ui.sh > /dev/null 2>&1
    if [[ $? -ne 0 ]]; then
        echo -e "${yellow}正在尝试使用 IPv4 获取 x-ui...${plain}"
        ${curl_bin} -4fLRo /usr/bin/x-ui https://raw.githubusercontent.com/charmtv/3x-ui/main/x-ui.sh > /dev/null 2>&1
        if [[ $? -ne 0 ]]; then
    ...
    if [[ $release == "alpine" ]]; then
        ...
        ${curl_bin} -fLRo /etc/init.d/x-ui https://raw.githubusercontent.com/charmtv/3x-ui/main/x-ui.rc > /dev/null 2>&1
        if [[ $? -ne 0 ]]; then
            ${curl_bin} -4fLRo /etc/init.d/x-ui https://raw.githubusercontent.com/charmtv/3x-ui/main/x-ui.rc > /dev/null 2>&1
            if [[ $? -ne 0 ]]; then
                _fail "错误：下载启动单元 x-ui.rc 失败，请确认服务器可以访问 GitHub"
            fi
        fi
        ...
        if [ -f "x-ui.service" ]; then
            ...
            if [ "$service_installed" = false ]; then
                ...
                case "${release}" in
                    ubuntu | debian | armbian)
                        ${curl_bin} -4fLRo ${xui_service}/x-ui.service https://raw.githubusercontent.com/charmtv/3x-ui/main/x-ui.service.debian > /dev/null 2>&1
                        ;;
                    arch | manjaro | parch)
                        ${curl_bin} -4fLRo ${xui_service}/x-ui.service https://raw.githubusercontent.com/charmtv/3x-ui/main/x-ui.service.arch > /dev/null 2>&1
                        ;;
                    *)
                        ${curl_bin} -4fLRo ${xui_service}/x-ui.service https://raw.githubusercontent.com/charmtv/3x-ui/main/x-ui.service.rhel > /dev/null 2>&1
                        ;;
                esac
